...
var _settings_base_text: String = ""
var _runtime_lines: Dictionary = {}
...
func update_settings_label() -> void:
	...
	if ProjectSettings.has_setting("application/config/version"):
		_settings_base_text += "Project Version: %s\n" % ProjectSettings.get_setting("application/config/version")
	...
	_settings_base_text += "Rendering Method: %s\n" % rendering_method_string
	...
	if viewport.get_camera_3d():
		...
		_settings_base_text += "3D scale (%s): %d%% = %d×%d" % [
	...
	_apply_settings_text()

func set_runtime_line(key: String, value: String) -> void:
	if value.is_empty():
		_runtime_lines.erase(key)
	else:
		_runtime_lines[key] = value
	_apply_settings_text()

func clear_runtime_line(key: String) -> void:
	_runtime_lines.erase(key)
	_apply_settings_text()

func get_lines_sorted() -> Array[String]:
	var keys := _runtime_lines.keys()
	keys.sort()
	var lines: Array[String] = []
	for key in keys:
		lines.append(str(_runtime_lines[key]))
	return lines

func _apply_settings_text() -> void:
	var text := _settings_base_text
	var lines := get_lines_sorted()
